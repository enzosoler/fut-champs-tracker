...
export default function AddMatchForm() {
  ...
  const endTypeConfig: Record<MatchEndType, { labelKey: EndKey; emoji: string; color: string }> = {
    ...
    abandoned:  { labelKey: 'end_abandoned',  emoji: '🚪',  color: 'border-gray-500 text-[#94A3B8]'     },
  };

  return (
    <form onSubmit={handleSubmit} className="min-h-screen bg-background text-white p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('week_id', lang)}</label>
            {activeWl ? (
              <div className="w-full bg-card border border-primary/40 rounded-xl px-4 py-2.5 text-sm flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-400 flex-shrink-0" />
                <span className="text-primary font-semibold">{activeWl}</span>
                <span className="text-[#94A3B8] text-xs ml-auto">{t('wl_active', lang)}</span>
              </div>
            ) : (
              <input type="text" value={weekId} onChange={e => setWeekId(e.target.value)}
                placeholder={t('week_id_hint', lang)}
                className="w-full bg-card border border-[#273246] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-white placeholder:text-[#94A3B8]/70" required />
            )}
          </div>
          <div className="space-y-1">
            <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('match_num', lang)}</label>
            <select value={matchNum} onChange={e => setMatchNum(Number(e.target.value))}
              className="w-full bg-card border border-[#273246] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-white">
        ...
        {/* Home / Away */}
        <div className="space-y-1">
          <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('home', lang)} / {t('away', lang)}</label>
            ...
            {(['Home', 'Away'] as const).map(v => (
              <button key={v} type="button" onClick={() => setHa(v)}
                className={`flex-1 py-2.5 rounded-xl border font-semibold text-sm transition ${
                  ha === v ? 'border-white bg-white/10 text-white' : 'border-[#273246] text-[#94A3B8] hover:border-white/30'
                }`}>
        ...
        <div className="bg-card border border-[#273246] rounded-2xl p-5 space-y-4">
          <p className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('score', lang)}</p>
              ...
              <div key={label} className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => set(Math.max(0, val - 1))}
                    className="w-9 h-9 rounded-full bg-muted flex items-center justify-center hover:bg-[#333]"><Minus size={14} /></button>
                  <span className={`text-4xl font-black w-12 text-center ${color}`}>{val}</span>
                  <button type="button" onClick={() => set(val + 1)}
                    className="w-9 h-9 rounded-full bg-muted flex items-center justify-center hover:bg-[#333]"><Plus size={14} /></button>
                </div>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#273246]/50">
            {[
              ...
              <div key={label} className="space-y-1">
                ...
                <input type="number" min="0" value={val} onChange={e => set(e.target.value)}
                  placeholder="—"
                  className="w-full bg-muted border border-[#273246] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white placeholder:text-gray-700" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('how_ended', lang)}</label>
            ...
            {(Object.entries(endTypeConfig) as [MatchEndType, typeof endTypeConfig[MatchEndType]][]).map(([key, cfg]) => (
              <button key={key} type="button" onClick={() => setMatchEndType(key)}
                className={`py-2 rounded-xl border text-sm font-semibold transition ${
                  matchEndType === key ? cfg.color + ' bg-white/5' : 'border-[#273246] text-[#94A3B8] hover:border-white/20'
                }`}>
        ...
        {/* Ping */}
        <div className="space-y-2">
          <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('connection', lang)}</label>
          <div className="flex gap-3">
            {(['Bom', 'Médio', 'Lag'] as const).map(p => (
              <button key={p} type="button" onClick={() => setPing(p)}
                className={`flex-1 py-2 rounded-xl border text-sm font-semibold transition ${
                  ping === p ? pingColor[p] + ' bg-white/5' : 'border-[#273246] text-[#94A3B8] hover:border-white/20'
                }`}>
        ...
        {/* Rage Quit */}
        <div className="space-y-2">
          <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('rage_quit', lang)}</label>
          <div className="flex gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('platform', lang)}</label>
          ...
          <div className="space-y-1">
            <label className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('opponent_tag', lang)}</label>
            <input type="text" value={gamertag} onChange={e => setGamertag(e.target.value)}
              placeholder="NomeDoOponente"
              className="w-full bg-card border border-[#273246] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-white placeholder:text-[#94A3B8]/70" />
          </div>
        </div>

        {/* Advanced Stats */}
        <button type="button" onClick={() => setShowAdvanced(v => !v)}
          className="w-full flex items-center justify-between bg-card border border-[#273246] rounded-xl px-4 py-3 text-sm text-[#94A3B8] hover:border-white/20 transition">
        ...
        {showAdvanced && (
          <div className="bg-card border border-[#273246] rounded-2xl p-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('xg_mine', lang)}</label>
                <input type="number" step="0.1" min="0" value={xgMe} onChange={e => setXgMe(e.target.value)} placeholder="1.4"
                  className="w-full bg-muted border border-[#273246] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white placeholder:text-gray-700" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('xg_opp', lang)}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('my_formation', lang)}</label>
                <select value={formationMe} onChange={e => setFormationMe(e.target.value)}
                  className="w-full bg-muted border border-[#273246] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white">
                  {FORMATIONS.map(f => <option key={f}>{f}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('opp_formation', lang)}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('my_possession', lang)}</label>
                <input type="number" min="0" max="100" value={possessionMe} onChange={e => setPossessionMe(e.target.value)} placeholder="55"
                  className="w-full bg-muted border border-[#273246] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white placeholder:text-gray-700" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-[#94A3B8]">{t('my_shots', lang)}</label>
        ...
        <div className="bg-card border border-[#273246] rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-[#94A3B8] uppercase tracking-wider">{t('auto_diff', lang)}</span>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[10px] text-[#94A3B8]/70">1</span>
                <span className="text-[10px] text-[#94A3B8]/70">10</span>
              </div>
            </div>
          ...
          <p className="text-[11px] text-[#94A3B8]/70 mt-2">{t('diff_tooltip', lang)}</p>
        </div>

        {/* Player Stats */}
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-[#94A3B8] uppercase tracking-wider">{t('player_stats', lang)}</h2>
          {squad.length === 0 ? (
            <p className="text-sm text-[#94A3B8] bg-card rounded-xl p-4 border border-[#273246]">
            ...
            <div className="space-y-2">
              {squad.map(player => {
                ...
                return (
                  <div key={player.id}
                    className={`rounded-xl border transition ${active ? 'border-white/30 bg-white/5' : 'border-[#273246] bg-card'}`}>
                    <button type="button" onClick={() => togglePlayer(player)}
                      ...
                      <span className="text-xs text-[#94A3B8] w-10 text-center border border-[#273246] rounded px-1 py-0.5">{player.position}</span>
                      <span className={`flex-1 text-sm font-medium ${active ? 'text-white' : 'text-[#94A3B8]'}`}>{player.name}</span>
                      <span className={`text-xs ${active ? 'text-white' : 'text-[#94A3B8]/70'}`}>
                      ...
                      <div className="px-4 pb-4 space-y-3">
                            ...
                            {(['goals', 'assists'] as const).map(field => (
                              <div key={field} className="flex items-center gap-2">
                                ...
                                <button type="button" onClick={() => adjustCounter(player.id, field, -1)}
                                  className="w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-[#333]"><Minus size={12} /></button>
                                <span className={`w-6 text-center font-bold ${field === 'goals' ? 'text-white' : 'text-blue-400'}`}>{entry[field]}</span>
                                <button type="button" onClick={() => adjustCounter(player.id, field, 1)}
                                  className="w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-[#333]"><Plus size={12} /></button>
                              </div>
                            ))}
                          </div>
                        )}
                        {isGK && (
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-[#94A3B8] w-16">🧤 {t('saves', lang)}</span>
                        ...
                        <div className="flex gap-3">
                          <button type="button" onClick={() => updateEntry(player.id, 'motm', !entry.motm)}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition ${
                              entry.motm ? 'border-yellow-400 bg-yellow-400/10 text-yellow-400' : 'border-[#273246] text-[#94A3B8] hover:border-white/20'
                          ...
                          <button type="button" onClick={() => updateEntry(player.id, 'clean_sheet', !entry.clean_sheet)}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition ${
                              entry.clean_sheet ? 'border-cyan-400 bg-cyan-400/10 text-cyan-400' : 'border-[#273246] text-[#94A3B8] hover:border-white/20'
        ...
        <div className="space-y-1">
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
            placeholder="..."
            className="w-full bg-card border border-[#273246] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-white resize-none placeholder:text-[#94A3B8]/70" />
        </div>
        ...
        <button type="submit" disabled={saving}
          className="w-full flex items-center justify-center gap-2 bg-primary text-black font-black py-4 rounded-2xl text-lg hover:bg-primary-dark transition disabled:opacity-50">
