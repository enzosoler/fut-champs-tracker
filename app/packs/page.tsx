...
export default function PacksPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Deletar este pack?')) return;
    setDeletingId(id);
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black">Packs & Recompensas</h1>
            <p className="text-sm text-gray-400 mt-0.5">Histórico de packs abertos</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-4 text-center">
            ...
            <p className="text-xs text-gray-400">Packs Abertos</p>
          </div>
          <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-4 text-center">
            <p className="text-2xl font-black text-[#FFB800]">{totalCoins.toLocaleString()}</p>
            <p className="text-xs text-gray-400">Total em Moedas</p>
          </div>
        </div>
        ...
        {showForm && (
          <div className="bg-[#1a1a1a] border border-white/20 rounded-2xl p-5 space-y-4">
            <h2 className="font-bold text-sm text-gray-300">Registrar Pack</h2>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs text-gray-400">Week ID (opcional)</label>
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400">Tipo de Pack</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs text-gray-400">Valor em Moedas</label>
                <input type="number" min="0" value={coinsValue} onChange={e => setCoinsValue(e.target.value)}
                  placeholder="15000"
                  className="w-full bg-[#262626] border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white placeholder:text-gray-600" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-gray-400">Data/Hora</label>
            ...
            <div className="space-y-1">
              <label className="text-xs text-gray-400">Pulls Notáveis (opcional)</label>
              <input type="text" value={notablePulls} onChange={e => setNotablePulls(e.target.value)}
                placeholder="ex: Mbappé 97, Vini Jr 95"
                className="w-full bg-[#262626] border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white placeholder:text-gray-600" />
            </div>
        ...
        {packs.length === 0 ? (
          <div className="text-center py-16 text-gray-500 space-y-3">
            ...
            <p>Nenhum pack registrado ainda.</p>
          </div>
        ) : (
          weeks.map(week => {
            ...
            return (
              <div key={week} className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-600">{weekPacks.length} packs</span>
                </div>
