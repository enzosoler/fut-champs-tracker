  ...
  useEffect(() => {
    async function load() {
      ...
      const summaries: OpponentSummary[] = Object.entries(map).map(([gamertag, matches]) => {
        ...
        return { gamertag, total: matches.length, wins, losses, draws, goalsFor, goalsAgainst, lastPlayed: matches[0].created_at, matches };
      });
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-white p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-black">{t('opponents_title', lang)}</h1>
          <p className="text-sm text-gray-400 mt-0.5">{t('opponents_sub', lang)}</p>
        </div>
        ...
        {opponents.length === 0 ? (
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <select value={sortBy} onChange={e => setSortBy(e.target.value as 'total' | 'wins' | 'losses')}
                className="bg-[#1a1a1a] border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-white">
                <option value="total">{t('most_played', lang)}</option>
                <option value="wins">{t('most_wins', lang)}</option>
                <option value="losses">{t('most_losses', lang)}</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-3">
                ...
                <p className="text-xs text-gray-400">{t('opponents_title', lang)}</p>
              </div>
              <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-3">
                  ...
                  <div key={opp.gamertag}
                    ...
                    <button type="button"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500">
                          {opp.total} {opp.total === 1 ? t('match', lang) : t('matches', lang)} ·
                          {t('goal_diff', lang)} {gd > 0 ? '+' : ''}{gd}
                        </p>
                      </div>
                      <div className="text-right mr-2">
                        <p className={`text-lg font-black ${winRateBadge(opp)}`}>{pct}%</p>
                        <p className="text-[10px] text-gray-600">{t('win_rate', lang)}</p>
                      </div>
                    ...
                    {isExpanded && (
                      <div className="border-t border-white/5 px-4 py-3 space-y-2">
                        <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('match_history_vs', lang)}</p>
                        {opp.matches.map(m => {
                          const r = getMatchResult(m);
                          return (
                            <div key={m.id} className="flex items-center gap-3 py-1">
                              ...
                              <div className="flex-1">
                                <span className="text-sm font-bold">
                                  ...
                                  {m.pk_me !== null && (
                                    <span className="text-gray-500 text-xs font-normal ml-1">({m.pk_me}–{m.pk_opp} pen)</span>
                                  )}
                                </span>
                              ...
                              <span className="text-[10px] text-gray-600">
                                {new Date(m.created_at).toLocaleDateString(localeMap[lang] ?? 'pt-BR', { day: '2-digit', month: 'short' })}
                              </span>
                            </div>
                          );
                        })}
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-white/5 text-center">
                          <div>
                            <p className="text-xs text-gray-500">{t('avg_scored', lang)}</p>
                            <p className="font-bold text-white">{(opp.goalsFor / opp.total).toFixed(1)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">{t('avg_conceded', lang)}</p>
                            <p className="font-bold text-red-400">{(opp.goalsAgainst / opp.total).toFixed(1)}</p>
                          </div>
                        </div>
                      </div>
                    )}
