...
export default function HistoryPage() {
  ...
  // Get unique week IDs for filter
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-black">Histórico <span className="text-white">de Partidas</span></h1>
          {matches.length > 0 && (
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(['All', 'W', 'D', 'L'] as const).map(r => (
            ...
            >
              {r === 'All' ? 'Todos' : r === 'W' ? 'Vitórias' : r === 'D' ? 'Empates' : 'Derrotas'}
            </button>
          ))}
          <select
            value={filterWeek}
        ...
        {filtered.length === 0 ? (
          ...
          Object.entries(grouped).map(([week, weekMatches]) => {
            ...
            const wL = weekMatches.filter(m => getMatchResult(m) === 'L').length;
            const wD = weekMatches.filter(m => getMatchResult(m) === 'D').length;
            return (
              <div key={week} className="space-y-2">
                ...
                <div className="flex items-center gap-3 pb-1">
                  ...
                  <div className="flex gap-1.5 text-xs">
                    <span className="text-white font-bold">{wW}V</span>
                    <span className="text-yellow-400 font-bold">{wD}E</span>
                    <span className="text-red-400 font-bold">{wL}D</span>
                  </div>
                ...
                {weekMatches.map(match => {
                  ...
                  const scorers = match.match_players.filter(p => p.goals > 0 || p.motm);
                  return (
                    <div
                      key={match.id}
                      ...
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            <span className={`text-xs px-1.5 py-0.5 rounded ${haColor[match.ha]}`}>{match.ha}</span>
                          ...
                          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                            {match.xg_me !== null && (
                              <>
                                <div className="flex justify-between">
                                  <span className="text-gray-500">xG (Meu)</span>
                                  <span className="font-semibold text-white">{match.xg_me?.toFixed(1)}</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-gray-500">xG (Opp)</span>
                            ...
                            {match.formation_me && (
                              <>
                                ...
                                <div className="flex justify-between">
                                  <span className="text-gray-500">Formação Opp</span>
                            ...
                            {match.shots_me !== null && (
                              <div className="flex justify-between">
                                <span className="text-gray-500">Finalizações</span>
                            ...
                            {match.match_end_type && match.match_end_type !== 'normal' && (
                              <div className="flex justify-between">
                                <span className="text-gray-500">Final</span>
                                <span className="font-semibold text-blue-400">
                                  {match.match_end_type === 'extra_time' ? '⏰ Prorrogação' :
                                   match.match_end_type === 'penalties'  ? '🥅 Penaltis'   :
                                   match.match_end_type === 'abandoned'  ? '🚪 Abandonada'  : ''}
                                </span>
                              </div>
                            )}
                              ...
                              <div className="space-y-1">
                                {match.match_players.map(mp => (
                                  <div key={mp.id} className="flex items-center gap-2 text-sm">
                                    ...
                                    {mp.goals > 0 && (
                                      <span className="text-white font-bold text-xs">{mp.goals}⚽</span>
                                    )}
                                    {mp.assists > 0 && (
                                      <span className="text-blue-400 font-bold text-xs">{mp.assists}🎯</span>
                                    )}
                                    {mp.motm && (
