...
export default function ProfilePage() {
  ...
  async function handleAvatarFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (uploadErr) {
      setError(`Upload failed: ${uploadErr.message}. Make sure the "avatars" bucket exists in Supabase Storage.`);
    ...
    else { setAvatarUrl(publicUrl); setSuccess('Avatar updated!'); }
    setUploadingAvatar(false);
    setTimeout(() => setSuccess(''), 3000);
  }
  ...
  async function removeAvatar() {
    ...
    if (error) setError(error.message);
    else { setAvatarUrl(''); setSuccess('Avatar removed.'); }
    setSaving(false);
    setTimeout(() => setSuccess(''), 3000);
  }
  ...
  async function changePassword() {
    ...
    if (error) setError(error.message);
    else { setSuccess('Password changed!'); setShowPwForm(false); setNewPassword(''); setConfirmPw(''); }
    setPwSaving(false);
    setTimeout(() => setSuccess(''), 3000);
  }
      ...
      <div className="px-4 space-y-5 pb-8">
        ...
        <div className="bg-card border border-[#273246] rounded-2xl p-6 flex flex-col items-center gap-4">
          ...
          {avatarUrl && (
            <button
              ...
              className="text-xs text-[#94A3B8] hover:text-loss transition underline"
            >
              Remove photo
            </button>
          )}
          ...
          <p className="text-[10px] text-[#94A3B8] text-center leading-relaxed">
            Upload a photo from your device. Requires the <span className="text-primary font-semibold">avatars</span> bucket in Supabase Storage.
          </p>
        </div>
        ...
        <div className="bg-card border border-[#273246] rounded-2xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest">Display Name</p>
            {!editingName && (
              <button
        ...
        <div className="bg-card border border-[#273246] rounded-2xl p-5">
          <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest mb-3">Email</p>
          <div className="flex items-center gap-3">
